...
VER	= $(subst ",,$(filter-out \#define CS_VERSION,$(shell grep CS_VERSION globals.h)))$(shell test -f /usr/bin/svnversion && svnversion -n . | awk 'BEGIN {FS = ":"} {print $1}' | sed 's/[MS]$$//' | sed 's/exported/0/' || echo -n 0 )
...
i386-pc-linux:
	@-$(MAKE) --no-print-directory \
		...
		OS_CULI="-lncurses" \
		OS_PTLI="-lpthread" \
		DS_OPTS="-O2 -DOS_LINUX -DCS_CONFDIR=${CS_CONFDIR} -Winline -Wall -Wno-implicit-function-declaration -Wno-parentheses  -finline-functions -fomit-frame-pointer -D'CS_SVN_VERSION="'"$(shell test -f /usr/bin/svnversion && svnversion -n . | awk 'BEGIN {FS = ":"} {print $1}' | sed 's/[MS]$$//' | sed 's/exported/0/' || echo -n 0 )"'"'" \
		DS_CFLAGS="-c" \
		DS_LDFLAGS="" \
		DS_ARFLAGS="-rvsl" \
		DS_CC=gcc \
...
i386-pc-linux-pcsc:
	@-$(MAKE) --no-print-directory \
		...
		DS_OPTS="-O2 -DOS_LINUX -DCS_CONFDIR=${CS_CONFDIR} -DHAVE_PCSC=1 -I/usr/include/PCSC -Winline -Wall -Wno-implicit-function-declaration -Wno-parentheses  -finline-functions -fomit-frame-pointer -D'CS_SVN_VERSION="'"$(shell test -f /usr/bin/svnversion && svnversion -n . | awk 'BEGIN {FS = ":"} {print $1}' | sed 's/[MS]$$//' | sed 's/exported/0/' || echo -n 0 )"'"'" \
		DS_CFLAGS="-c" \
		DS_LDFLAGS="" \
		DS_ARFLAGS="-rvsl" \
		DS_CC=gcc \
...
macosx-native:
	@-$(MAKE) --no-print-directory \
		...
		OS_CULI="-lncurses" \
		OS_PTLI="-lpthread" \
		DS_OPTS="-O2 -DOS_MACOSX -DNEED_DAEMON -DCS_NOSHM -DHAVE_PTHREAD_H -DUSE_PTHREAD -DCS_CONFDIR=${CS_CONFDIR} -DHAVE_PCSC=1 -Winline -Wall -Wno-implicit-function-declaration -Wno-parentheses  -finline-functions -fomit-frame-pointer -D'CS_SVN_VERSION="'"$(shell test -f /usr/bin/svnversion && svnversion -n . | awk 'BEGIN {FS = ":"} {print $1}' | sed 's/[MS]$$//' | sed 's/exported/0/' || echo -n 0 )"'"'" \
		DS_CFLAGS="-c" \
...
i386-pc-freebsd:
	@-$(MAKE) --no-print-directory \
		...
		OS_CULI="-lncurses" \
		OS_PTLI="-lpthread" \
		DS_OPTS="-O2 -DOS_FREEBSD -DBSD_COMP  -DCS_CONFDIR=${CS_CONFDIR} -static-libgcc -D'CS_SVN_VERSION="'"$(shell test -f /usr/bin/svnversion && svnversion -n . | awk 'BEGIN {FS = ":"} {print $1}' | sed 's/[MS]$$//' | sed 's/exported/0/' || echo -n 0 )"'"'" \
		DS_CFLAGS="-c" \
		DS_LDFLAGS="" \
		DS_ARFLAGS="-rvsl" \
		DS_CC=gcc \
...
cross-powerpc-tuxbox-linux:
	@-$(MAKE) --no-print-directory \
		...
		OS_CULI="-lncurses" \
		OS_PTLI="-lpthread" \
		DS_OPTS="-O2 -DOS_LINUX -DTUXBOX -DPPC -DCS_CONFDIR='\"/var/tuxbox/config\"' -D'CS_SVN_VERSION="'"$(shell test -f /usr/bin/svnversion && svnversion -n . | awk 'BEGIN {FS = ":"} {print $1}' | sed 's/[MS]$$//' | sed 's/exported/0/' || echo -n 0 )"'"'" \
		DS_CFLAGS="-c" \
		DS_LDFLAGS="" \
		DS_ARFLAGS="-rvsl" \
...
cross-sh4-linux:
	@-$(MAKE) --no-print-directory \
		...
		OS_CULI="-lncurses" \
		OS_PTLI="-lpthread" \
		DS_OPTS="-O2 -DOS_LINUX -DSH4 -DTUXBOX -DCS_CONFDIR='\"/var/tuxbox/config\"' -D'CS_SVN_VERSION="'"$(shell test -f /usr/bin/svnversion && svnversion -n . | awk 'BEGIN {FS = ":"} {print $1}' | sed 's/[MS]$$//' | sed 's/exported/0/' || echo -n 0 )"'"'" \
		DS_CFLAGS="-c" \
		DS_LDFLAGS="" \
		DS_ARFLAGS="-rvsl" \
...
cross-i386-pc-cygwin:
	@-$(MAKE) --no-print-directory \
		...
		OS_CULI="-lncurses" \
		OS_PTLI="-lpthread" \
		DS_OPTS="-O2 -DOS_CYGWIN32 -DCS_CONFDIR=${CS_CONFDIR} -static -D'CS_SVN_VERSION="'"$(shell test -f /usr/bin/svnversion && svnversion -n . | awk 'BEGIN {FS = ":"} {print $1}' | sed 's/[MS]$$//' | sed 's/exported/0/' || echo -n 0 )"'"'" \
		DS_CFLAGS="-c" \
		DS_LDFLAGS="" \
		DS_ARFLAGS="-rvsl" \
...
i386-pc-cygwin:
	@-$(MAKE) --no-print-directory \
		...
		OS_CULI="-lncurses" \
		OS_PTLI="-lpthread" \
		DS_OPTS="-O2 -DOS_CYGWIN32 -DCS_CONFDIR=${CS_CONFDIR} -I /tmp/include -D'CS_SVN_VERSION="'"$(shell test -f /usr/bin/svnversion && svnversion -n . | awk 'BEGIN {FS = ":"} {print $1}' | sed 's/[MS]$$//' | sed 's/exported/0/' || echo -n 0 )"'"'" \
		DS_CFLAGS="-c" \
		DS_LDFLAGS="" \
		DS_ARFLAGS="-rvsl" \
		DS_CC=gcc \
...
cross-sparc-sun-solaris2.7:
	@-$(MAKE) --no-print-directory \
		...
		OS_CULI="-lncurses" \
		OS_PTLI="-lpthread" \
		DS_OPTS="-O2 -DOS_SOLARIS -DOS_SOLARIS7 -DBSD_COMP -DCS_CONFDIR=${CS_CONFDIR} -static-libgcc -D'CS_SVN_VERSION="'"$(shell test -f /usr/bin/svnversion && svnversion -n . | awk 'BEGIN {FS = ":"} {print $1}' | sed 's/[MS]$$//' | sed 's/exported/0/' || echo -n 0 )"'"'" \
		DS_CFLAGS="-c" \
		DS_LDFLAGS="-lsocket" \
		DS_ARFLAGS="-rvsl" \
...
cross-rs6000-ibm-aix4.2:
	@-$(MAKE) --no-print-directory \
		...
		OS_CULI="-lncurses" \
		OS_PTLI="-lpthreads" \
		DS_OPTS="-O2 -DOS_AIX -DOS_AIX42 -DCS_CONFDIR=${CS_CONFDIR} -static-libgcc -D'CS_SVN_VERSION="'"$(shell test -f /usr/bin/svnversion && svnversion -n . | awk 'BEGIN {FS = ":"} {print $1}' | sed 's/[MS]$$//' | sed 's/exported/0/' || echo -n 0 )"'"'" \
		DS_CFLAGS="-c" \
		DS_LDFLAGS="" \
		DS_ARFLAGS="-rvsl" \
...
cross-mips-sgi-irix6.5:
	@-$(MAKE) --no-print-directory \
		...
		OS_CULI="-lncurses" \
		OS_PTLI="-lpthread" \
		DS_OPTS="-O2 -DOS_IRIX -DOS_IRIX65 -DCS_CONFDIR=${CS_CONFDIR} -static-libgcc -D'CS_SVN_VERSION="'"$(shell test -f /usr/bin/svnversion && svnversion -n . | awk 'BEGIN {FS = ":"} {print $1}' | sed 's/[MS]$$//' | sed 's/exported/0/' || echo -n 0 )"'"'" \
		DS_CFLAGS="-c" \
		DS_LDFLAGS="" \
		DS_ARFLAGS="-rvsl" \
...
cross-mipsel-router-linux-uclibc927:
	...
	@-$(MAKE) --no-print-directory \
		...
		OS_CULI="-lncurses" \
		OS_PTLI="-lpthread" \
		DS_OPTS="-O2 -DOS_LINUX -DMIPSEL -DUCLIBC -DUSE_GPIO -DCS_CONFDIR=${CS_CONFDIR} -static-libgcc -D'CS_SVN_VERSION="'"$(shell test -f /usr/bin/svnversion && svnversion -n . | awk 'BEGIN {FS = ":"} {print $1}' | sed 's/[MS]$$//' | sed 's/exported/0/' || echo -n 0 )"'"'" \
		DS_CFLAGS="-c" \
		DS_LDFLAGS="" \
		DS_ARFLAGS="-rvsl" \
...
cross-mipsel-fonera2:
	@-$(MAKE) --no-print-directory \
		...
		OS_CULI="-lncurses" \
		OS_PTLI="-lpthread" \
		DS_OPTS="-Iopenssl-include -O2 -DOS_LINUX -DMIPSEL -DUCLIBC -DCS_CONFDIR=${CS_CONFDIR} -static-libgcc -D'CS_SVN_VERSION="'"$(shell test -f /usr/bin/svnversion && svnversion -n . | awk 'BEGIN {FS = ":"} {print $1}' | sed 's/[MS]$$//' | sed 's/exported/0/' || echo -n 0 )"'"'" \
		DS_CFLAGS="-c" \
		DS_LDFLAGS="" \
		DS_ARFLAGS="-rvsl" \
...
cross-mipsel-tuxbox-linux-glibc:
	@-$(MAKE) --no-print-directory \
		...
		OS_CULI="-lncurses" \
		OS_PTLI="-lpthread" \
		DS_OPTS="-O2 -DOS_LINUX -DTUXBOX -DMIPSEL -DCS_CONFDIR='\"/var/tuxbox/config\"' -static-libgcc -D'CS_SVN_VERSION="'"$(shell test -f /usr/bin/svnversion && svnversion -n . | awk 'BEGIN {FS = ":"} {print $1}' | sed 's/[MS]$$//' | sed 's/exported/0/' || echo -n 0 )"'"'" \
		DS_CFLAGS="-c" \
		DS_LDFLAGS="" \
		DS_ARFLAGS="-rvsl" \
...
hppa1.1-hp-hpux10.20:
	@-$(MAKE) --no-print-directory \
		...
		OS_CULI="-lncurses" \
		OS_PTLI="-lpthread" \
		DS_OPTS="-O2 -DOS_HPUX -DOS_HPUX10 -D_XOPEN_SOURCE_EXTENDED -DCS_CONFDIR=${CS_CONFDIR} -D'CS_SVN_VERSION="'"$(shell test -f /usr/bin/svnversion && svnversion -n . | awk 'BEGIN {FS = ":"} {print $1}' | sed 's/[MS]$$//' | sed 's/exported/0/' || echo -n 0 )"'"'" \
		DS_CFLAGS="-c" \
		DS_LDFLAGS="" \
		DS_ARFLAGS="-rvsl" \
		DS_CC=gcc \
...
alpha-dec-osf5.1:
	@-$(MAKE) --no-print-directory \
		...
		OS_CULI="-lncurses" \
		OS_PTLI="-lpthread" \
		DS_OPTS="-O2 -DOS_OSF -DOS_OSF5 -DCS_CONFDIR=${CS_CONFDIR} -D'CS_SVN_VERSION="'"$(shell test -f /usr/bin/svnversion && svnversion -n . | awk 'BEGIN {FS = ":"} {print $1}' | sed 's/[MS]$$//' | sed 's/exported/0/' || echo -n 0 )"'"'" \
...
cross-arm-nslu2-linux:
	@-$(MAKE) --no-print-directory \
		...
		OS_CULI="-lncurses" \
		OS_PTLI="-lpthread" \
		DS_OPTS="-DOS_LINUX -O2 -DARM -DALIGNMENT -DCS_CONFDIR=${CS_CONFDIR} -D'CS_SVN_VERSION="'"$(shell test -f /usr/bin/svnversion && svnversion -n . | awk 'BEGIN {FS = ":"} {print $1}' | sed 's/[MS]$$//' | sed 's/exported/0/' || echo -n 0 )"'"'" \
		DS_CFLAGS="-c" \
		DS_LDFLAGS="" \
		DS_ARFLAGS="-rvsl" \
		DS_AWK="awk" \
