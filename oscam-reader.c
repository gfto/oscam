...
void casc_check_dcw(struct s_reader * reader, int32_t idx, int32_t rc, uchar *cw)
{
	...
	char ecmd5[17*3];
	cs_hexdump(0, ecm->ecmd5, 16, ecmd5, sizeof(ecmd5));
	cs_debug_mask(D_TRACE, "received cw from %s task %d idx %d ecm hash=%s", 
		reader->label, idx, ecm->idx, ecmd5);
   	...
   	if (ecm->parent) {                
		...
		cs_log("invalid ecm idx received! reader %s idx %d",reader->label, idx);
