...
static int do_cmd(struct s_reader * reader, const unsigned char *ins, const unsigned char *txbuff, unsigned char *rxbuff, unsigned char * cta_res, unsigned short * cta_length)
{
  def_resp2;
...
static void read_tiers(struct s_reader * reader)
{
  ...
  int l;
  l=do_cmd(reader, ins2a,NULL,NULL,cta_res,&cta_lr);
  ...
  for(i=0; i<num; i++) {
    ins76[2]=i;
    l=do_cmd(reader, ins76,NULL,NULL,cta_res,&cta_lr);
    if(l<0 || !status_ok(cta_res+l)) return;
...
/*    else
  ...
  if(do_cmd(reader, ins7416, NULL, NULL,cta_res,&cta_lr)<0) {
  ...
  if (reader->boxid > 0) {
    ...
    int boxidOK=0;
    l=do_cmd(reader, ins36, NULL, buff,cta_res,&cta_lr);
    if(l>=0) {
      int i;
      for(i=0; i<l ;i++) {
  ...
  l=do_cmd(reader, ins58, NULL, buff,cta_res,&cta_lr);
  if(l<0) {
  ...
  l=do_cmd(reader, insB4, tbuff, NULL,cta_res,&cta_lr);
  ...
  l=do_cmd(reader, insBC, NULL, NULL,cta_res,&cta_lr);
  if(l<0) {
  ...
  l=do_cmd(reader, insBE, NULL, NULL,cta_res,&cta_lr);
  if(l<0) {
  ...
  l=do_cmd(reader, ins58a, NULL, NULL,cta_res,&cta_lr);
  if(l<0) {
  ...
  l=do_cmd(reader, ins4Ca,payload4C, NULL,cta_res,&cta_lr);
  if(l<0 || !status_ok(cta_res)) {
...
int videoguard_do_ecm(struct s_reader * reader, ECM_REQUEST *er)
{
  ...
  ins40[4]=lenECMpart2;
  int l;
  l = do_cmd(reader, ins40,tbuff,NULL,cta_res,&cta_lr);
  if(l>0 && status_ok(cta_res)) {
    l = do_cmd(reader, ins54,NULL,NULL,cta_res,&cta_lr);
...
int videoguard_do_emm(struct s_reader * reader, EMM_PACKET *ep)
{
  ...
  while (payload) {
    ins42[4]=*payload;
    int l = do_cmd(reader, ins42,payload+1,NULL,cta_res,&cta_lr);
    if(l>0 && status_ok(cta_res)) {
      rc=OK;
      }
