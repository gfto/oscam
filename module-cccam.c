...
void *llist_append(LLIST *l, void *o)
{
	...
	pthread_mutex_lock(&l->lock);
	ln = calloc(1, sizeof(struct llist_node));
	if (!ln)
	{
...
{
    if (!itr || !itr->l || itr->l->items==0) return NULL;
    itr->l->items--;
...
//  pthread_mutex_lock(&cc->lock);
    ...
    if (cc->cur_card)
    {
	//uint8 *ecmbuf = calloc(1, cur_er->l+13);
	//if (ecmbuf==NULL) return -1;
...
static cc_msg_type_t cc_parse_msg(uint8 *buf, int l)
{
    ...
    case MSG_NEW_CARD:
	{
	    ...
	    if (buf[14] > reader[ridx].cc_maxhop) break;
	    
	    card = calloc(1, sizeof(struct cc_card));
	    if (!card) break;
    ...
    case MSG_CW_NOK2:
	{
	    //cs_debug("cccam: cw nok, sid = %04X", cc->cur_sid);
	    int f = 0;
	    LLIST_ITR itr;
	...
	{
	    ECM_REQUEST *er;
	    cc->cur_card = calloc(1, sizeof(struct cc_card));
	    if (!cc->cur_card) break;
...
static int cc_recv_chk(uchar *dcw, int *rc, uchar *buf)
{
	...
	*rc = 1;
	...
	if ((buf[1] == (MSG_CW_NOK1)) || (buf[1] == (MSG_CW_NOK2)))
	{
	    memset(dcw, 0, 16);
	    *rc = 0;
	    return (0);
...
static int cc_cli_connect(void)
{
    ...
    cc = calloc(1, sizeof(struct cc_data));
    if (cc==NULL)
    {
    ...
    // get init seed
    n = recv(handle, data, 16, MSG_WAITALL);
    if (n != 16)
    {
    ...
    n = recv(handle, data, 20, MSG_WAITALL);
    if (n != 20)
    {
