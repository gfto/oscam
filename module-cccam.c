...
//From Project:KeyNation:
static unsigned char init_table[256] = {
...
void handle_cmd_0b(uint8 *in)
{
	AES_KEY key;
	memset(&key, 0, sizeof(key));
	uint8 out[16];
	AES_set_decrypt_key((unsigned char *) &init_table, 256, &key);
	AES_decrypt((unsigned char *)in, (unsigned char *) &out, &key);
