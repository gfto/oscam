...
static int cc_msg_recv(uint8 *buf) {
	...
	if (((netbuf[2] << 8) | netbuf[3]) != 0) { // check if any data is expected in msg
		...
		len = recv(handle, netbuf + 4, (netbuf[2] << 8) | netbuf[3],
...
static int cc_send_ecm(ECM_REQUEST *er, uchar *buf) {
	...
	if (!reader[ridx].cc_disable_auto_block && cc_is_auto_blocked(
				cc->auto_blocked, cur_er->caid, cur_er->prid, cur_er->srvid, 60*60*1)) { //TODO: Timeout 60*60*1 = 1h, Config?
...
static cc_msg_type_t cc_parse_msg(uint8 *buf, int l) {
	...
	case MSG_CARD_REMOVED: {
		...
		if (!found) {
			if (!reader[ridx].caid[0])
				cs_debug("cccam: card removed %08x not found?, count %d", b2i(
				...
				if (sid) {
					*sid = cc->cur_sid;
					...
					cs_log("%s added sid block %04X for card %08x",
							getprefix(), cc->cur_sid, cc->cur_card->id);
				}
				int n;
				if (!reader[ridx].cc_disable_retry_ecm && (n = cc_get_nxt_ecm()) >= 0) {
	...
	case MSG_CW_ECM:
		if (is_server) {
			...
			cc->cur_card->id = buf[10] << 24 | buf[11] << 16 | buf[12] << 8
					| buf[13];
...
static int cc_recv_chk(uchar *dcw, int *rc, uchar *buf) {
	...
	if (buf[1] == MSG_CW_ECM) {
		...
		cs_debug("cccam: recv chk - MSG_CW %d - %s", cc->recv_ecmtask, cs_hexdump(0,
				dcw, 16));
		*rc = 1;
...
int cc_cli_init() {
	if (!reader[ridx].tcp_connected) {
		...
		cs_log("proxy reader: %s (%s:%d) cccam v%s build %s, maxhop: %d, retry ecm: %d, auto block: %d",
				reader[ridx].label, reader[ridx].device, reader[ridx].r_port,
				reader[ridx].cc_version, reader[ridx].cc_build, reader[ridx].cc_maxhop,
				!reader[ridx].cc_disable_retry_ecm, !reader[ridx].cc_disable_auto_block);
