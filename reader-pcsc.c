...
int pcsc_reader_init(struct s_reader *pcsc_reader, char *device)
{
		...
		if ( rv == SCARD_S_SUCCESS ) {
			...
			cs_debug("PCSC initializing result (%lx) protocol (T=%lx)", rv, pcsc_reader->dwActiveProtocol );
            if (rv==SCARD_S_SUCCESS) {
...
int pcsc_reader_do_api(struct s_reader *pcsc_reader, uchar *buf, uchar *cta_res, ushort *cta_lr,int l, int dbg)
{
     ...
     *cta_lr=dwRecvLength;
     ...
     cs_debug("PCSC doapi (%lx ) (T=%d)", rv, pcsc_reader->dwActiveProtocol );
     if ( rv  == SCARD_S_SUCCESS ){
         return OK;
     }
     else {
...
int pcsc_activate_card(struct s_reader *pcsc_reader, uchar *atr, ushort *atr_size)
{
    ...
    cs_debug("PCSC Protocol (T=%d)",pcsc_reader->dwActiveProtocol);
        ...
        if(rv==SCARD_E_INVALID_HANDLE){
		 ...
		 if (rv == SCARD_S_SUCCESS && (dwState & (SCARD_PRESENT | SCARD_NEGOTIABLE | SCARD_POWERED ) )) {
			 ...
			 if ( (rv==SCARD_W_RESET_CARD) && (dwState == 0) ) {
				 ...
				 cs_debug("PCSC card inserted FAILURE in %s (%lx) card state (%x) (T=%d)", pcsc_reader->pcsc_name, rv, dwState, pcsc_reader->dwActiveProtocol);
			 }
			 return 0;
		 }
