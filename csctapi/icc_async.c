	 ...
	 case ATR_PROTOCOL_TYPE_T1:
			{
				...
				if (reader->mhz > 2000) {
					reader->BWT = (uint16_t) ((((1<<bwi) * 960L * 372L / ((double)reader->mhz / (double) reader->divider / 100L) + 11L)/1000L) * (double) reader->current_baudrate / 1000L); // BWT in ETU otherwise it wont fit in uint16_t
				}
