...
int32_t Protocol_T14_ExchangeTPDU(struct s_reader *reader, unsigned char *cmd_raw, uint16_t command_len, unsigned char *rsp, uint16_t *lr)
{
	...
	*lr = 0; //in case of error this is returned
	...
	if(ICC_Async_Receive(reader, 8, buffer, 0, timeout) != OK) { rdr_log(reader, " DAMNED STILL T14 receive timeout error 1"); return ERROR; }   //Read one procedure byte
	recved = (int32_t)buffer[7];
	if(recved)
	{
		timeout = ICC_Async_GetTimings(reader, reader->read_timeout);  // we are going to receive: WWT timeout
		if ((reader->typ == R_SMART) && (reader->smart_type >= 2)) timeout *= 1.1;
		if(ICC_Async_Receive(reader, recved, buffer + 8, 0 , timeout) != OK) { rdr_log(reader, " DAMNED STILL T14 receive timeout error 2"); return ERROR; }
	}
	timeout = ICC_Async_GetTimings(reader, reader->read_timeout);  // we are going to receive: WWT timeout
	if ((reader->typ == R_SMART) && (reader->smart_type >= 2)) timeout *= 1.1;
	if(ICC_Async_Receive(reader, 1, &ixor, 0, timeout) != OK) { rdr_log(reader, " DAMNED STILL T14 receive timeout error 3"); return ERROR; }
